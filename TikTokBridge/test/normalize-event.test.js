...
test('normalizes direct TikTok v3 chat content field', () => {
    const event = normalizeChat({
        event: { msgId: 'chat-v3-1' },
        user: { userId: '99', uniqueId: 'viewer99', nickname: 'Viewer 99' },
        content: 'hey'
    });

    assert.equal(event.comment, 'hey');
    assert.equal(event.uniqueId, 'viewer99');
});
