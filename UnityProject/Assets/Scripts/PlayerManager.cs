...
namespace TikTokLiveGame
{
    public sealed class PlayerManager : MonoBehaviour
    {
        ...
        private float nextExpiryScanAt;
        ...
        public event System.Action SnapshotApplied;
        ...
        public void Handle(TikTokEvent data)
        {
            ...
            {
                if (snapshotRoutine != null) StopCoroutine(snapshotRoutine);
                snapshotRoutine = StartCoroutine(ApplySnapshotBatched(data.players ?? System.Array.Empty<TikTokPlayerData>()));
                return;
            }
        ...
        // Điều kiện cảnh áp cho MỌI hành động, không riêng cánh / huy hiệu:
        // rule đặt "Chỉ khi đang Top 1-3" thì hành động chỉ chạy cho người đang trong Top.
        public static bool ActionAllowed(TikTokEvent data, PlayerActor actor)
        {
            if (data == null) return false;
            if (data.conditionEvaluated) return data.conditionAllowed;

            bool topRequired = string.Equals(data.decorationCondition, "top_only", System.StringComparison.OrdinalIgnoreCase);
            // Xét theo thứ hạng ngay trước sự kiện, đúng như cách cánh / huy hiệu vẫn xét:
            // gift vừa gửi chưa được tính vào bảng Top.
            data.conditionAllowed = !topRequired || (actor != null && actor.IsTopRanked);
            data.conditionEvaluated = true;
        ...
        private static void ApplyAction(PlayerActor actor, TikTokEvent data, float duration)
        {
            ...
            if (data.action is "medal" or "vip" or "topdj")
            {
                actor.ShowDecoration(
                    string.IsNullOrWhiteSpace(data.label) ? "VIP" : data.label,
                    data.variant,
                    Mathf.Max(3f, duration));
            }
        }
        ...
        private PlayerActor Create(TikTokPlayerData data)
        {
            ...
            int slot = AllocateCrowdSlot();
            crowdSlots[data.userId] = slot;
        ...
        {
            if (occupiedSlots.Count >= CrowdCapacity) return Random.Range(0, CrowdCapacity);

            int minimumCellDistanceSquared = occupiedSlots.Count < 30 ? 9 : occupiedSlots.Count < 80 ? 4 : occupiedSlots.Count < 140 ? 2 : 1;
            for (int attempt = 0; attempt < 48; attempt++)
            {
                int candidate = Random.Range(0, CrowdCapacity);
                if (occupiedSlots.Contains(candidate)) continue;
                if (IsSeparatedSlot(candidate, minimumCellDistanceSquared)) return candidate;
            }

            int start = Random.Range(0, CrowdCapacity);
            for (int offset = 0; offset < CrowdCapacity; offset++)
            {
                int candidate = (start + offset) % CrowdCapacity;
                if (!occupiedSlots.Contains(candidate)) return candidate;
            }
            return 0;
        }

        private bool IsSeparatedSlot(int candidate, int minimumCellDistanceSquared)
        {
            int candidateRow = candidate / CrowdColumns;
            int candidateColumn = candidate % CrowdColumns;
            foreach (int other in occupiedSlots)
            {
                int rowDelta = candidateRow - other / CrowdColumns;
                int columnDelta = candidateColumn - other % CrowdColumns;
                if (rowDelta * rowDelta + columnDelta * columnDelta < minimumCellDistanceSquared) return false;
            }
            return true;
        }

        private System.Collections.IEnumerator ApplySnapshotBatched(TikTokPlayerData[] snapshot)
        {
            Clear(false);
            ...
            for (int index = 0; index < snapshot.Length; index++)
            {
                ...
                if ((index + 1) % batchSize == 0) yield return null;
            }
            layoutDirty = true;
            visibilityDirty = true;
            nextNpcRefillAt = 0f;
        ...
        private void Update()
        {
            if (Time.unscaledTime >= nextExpiryScanAt)
            {
                nextExpiryScanAt = Time.unscaledTime + 2f;
                float cutoff = Time.unscaledTime - playerTtlSeconds;
                List<string> expired = null;
                foreach (KeyValuePair<string, PlayerActor> pair in players)
                {
                    if (pair.Value.IsNpc || pair.Value.LastActiveTime >= cutoff) continue;
                    expired ??= new List<string>();
                    expired.Add(pair.Key);
                }
                if (expired != null) foreach (string id in expired) Remove(id);
            }
            if (snapshotRoutine == null && Time.unscaledTime >= nextNpcRefillAt) EnsureNpcCrowd();
        }
        ...
        private void Clear(bool refillNpcs)
        {
            ...
            layoutDirty = false;
            visibilityDirty = false;
            nextNpcRefillAt = refillNpcs ? 0f : float.PositiveInfinity;
            if (refillNpcs) EnsureNpcCrowd();
        }
        ...
        private void Remove(string id)
        {
            ...
            if (crowdSlots.Remove(id, out int releasedSlot)) occupiedSlots.Remove(releasedSlot);
        ...
        private void RemoveOneNpc()
        {
            ...
            if (crowdSlots.Remove(npcId, out int releasedSlot)) occupiedSlots.Remove(releasedSlot);
