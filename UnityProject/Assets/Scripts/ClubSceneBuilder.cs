...
namespace TikTokLiveGame
{
    public static class ClubSceneBuilder
    {
        ...
        private static Texture2D currentBackgroundTexture;
        ...
        public static void Build()
        {
            ...
            string path = ResolveLiveAssetPath("nenamphu.png");
            if (!string.IsNullOrWhiteSpace(path) && File.Exists(path))
            {
                if (tex.LoadImage(File.ReadAllBytes(path)))
                {
                    // Bilinear keeps the vertical artwork sharp enough without the washed
                    // appearance that can come from a missing/default texture.
                    tex.filterMode = FilterMode.Bilinear;
                    tex.anisoLevel = 4;
                    tex.wrapMode = TextureWrapMode.Clamp;
                    tex.Apply(false, true);
                    Debug.Log($"[TISO] Background loaded: {path} ({tex.width}x{tex.height})");
                }
                else
                {
                    Debug.LogError($"[TISO] Could not decode background image: {path}");
                }
            }
            else
            {
                // Never leave Unity's uninitialised 2x2 texture visible: it looks like a
                // flat grey wall and makes the whole scene appear foggy/washed out.
                tex.SetPixels(new[]
                {
                    new Color(0.01f, 0.005f, 0.025f), new Color(0.01f, 0.005f, 0.025f),
                    new Color(0.01f, 0.005f, 0.025f), new Color(0.01f, 0.005f, 0.025f)
                });
                tex.Apply(false, true);
                Debug.LogError("[TISO] Missing LiveAssets/nenamphu.png. Background fallback is active.");
            }
        ...
        public static bool ReloadBackground()
        {
            string path = ResolveLiveAssetPath("nenamphu.png");
            if (string.IsNullOrWhiteSpace(path) || !File.Exists(path))
            {
                Debug.LogError("[TISO] Reload background failed: LiveAssets/nenamphu.png not found.");
                return false;
            }
            ...
            {
                Debug.LogError($"[TISO] Reload background read failed: {ex.Message}");
                return false;
            }

            Texture2D next = new Texture2D(2, 2, TextureFormat.RGBA32, false);
            if (!next.LoadImage(bytes))
            {
                Object.Destroy(next);
                Debug.LogError($"[TISO] Reload background decode failed: {path}");
                return false;
            }
            next.filterMode = FilterMode.Bilinear;
            next.anisoLevel = 4;
            next.wrapMode = TextureWrapMode.Clamp;
            next.Apply(false, true);

            GameObject root = GameObject.Find("AmPhuBackdrop_Room");
            if (root == null)
            {
                Object.Destroy(next);
                Debug.LogError("[TISO] Reload background failed: backdrop root not found.");
                return false;
            }
            ...
            foreach (Renderer renderer in root.GetComponentsInChildren<Renderer>(true))
            {
                ...
                renderer.material.mainTexture = next;
            }

            Texture2D previous = currentBackgroundTexture;
            currentBackgroundTexture = next;
            if (previous != null && previous != next) Object.Destroy(previous);
            Debug.Log($"[TISO] Background reloaded: {path} ({next.width}x{next.height})");
            return true;
        }

        private static string ResolveLiveAssetPath(string fileName)
        {
            string[] candidates =
            {
                // Editor: <root>/UnityProject/Assets -> ../../LiveAssets
                // Build:  <root>/Build/TISO_Data -> ../../LiveAssets
                Path.GetFullPath(Path.Combine(Application.dataPath, "..", "..", "LiveAssets", fileName)),
                Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "LiveAssets", fileName)),
                Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "..", "LiveAssets", fileName))
            };
