...
namespace TikTokLiveGame
{
    public sealed class PlayerActor : MonoBehaviour
    {
        ...
        private float forcedDecorationUntil;
        private string forcedDecorationVariant = string.Empty;
        ...
        public void ShowDecoration(string label, float seconds)
        {
            ShowDecoration(label, string.Empty, seconds);
        }

        public void ShowDecoration(string label, string variant, float seconds)
        {
            if (titleLabel == null) return;
            float visibleSeconds = Mathf.Max(3f, seconds);
            SetTitleText(FormatBannerTitle(string.IsNullOrWhiteSpace(label) ? "VIP" : label));
            SetTitleVisible(true);
            forcedDecorationUntil = Time.time + visibleSeconds;
            forcedDecorationVariant = NormalizeForcedDecorationVariant(variant);
            RefreshDecorations();
            CancelInvoke(nameof(ApplyEvolution));
            Invoke(nameof(ApplyEvolution), visibleSeconds);
        }

        private static string NormalizeForcedDecorationVariant(string variant)
        {
        ...
        private void RefreshDecorations()
        {
            ...
            bool forcedDecoration = Time.time < forcedDecorationUntil;
            bool decorated = giftFocused || topRank > 0 || GiftPower >= 10 || forcedDecoration;
            string rankedVariant = topRank == 1 || GiftPower >= 1000 ? "royal"
                : topRank == 2 || GiftPower >= 200 ? "fire"
                : topRank == 3 || GiftPower >= 50 ? "neon"
                : "ice";
            string variant = forcedDecoration && !string.IsNullOrEmpty(forcedDecorationVariant)
                ? forcedDecorationVariant
                : rankedVariant;
            ...
            string wingsAsset = giftFocused || variant is "aura" or "royal"
                ? "Banners/energy-wings-v2"
                : $"Banners/wings-{variant}";
            wingsRenderer.sprite = Resources.Load<Sprite>(wingsAsset) ?? Resources.Load<Sprite>("Banners/energy-wings-v2");
            string bannerVariant = variant == "aura" ? "ice" : variant;
            bannerRenderer.sprite = Resources.Load<Sprite>($"Banners/title-{bannerVariant}") ?? Resources.Load<Sprite>("Banners/title-ice");
            ...
            wingsRenderer.enabled = decorated && wingsRenderer.sprite != null;
