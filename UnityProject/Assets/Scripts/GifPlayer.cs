    ...
    public sealed class GifPlayer : MonoBehaviour
    {
        ...
        private IEnumerator LoadFramesAsync()
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                // PNG decoding happens on Unity's main thread. Yield frequently so Windows
                // keeps receiving window messages instead of marking the game Not Responding.
                if ((i + 1) % Mathf.Max(1, framesPerLoadYield) == 0) yield return null;
            }
