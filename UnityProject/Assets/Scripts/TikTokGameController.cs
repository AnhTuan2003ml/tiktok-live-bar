...
namespace TikTokLiveGame
{
    public sealed class TikTokGameController : MonoBehaviour
    {
        ...
        public bool IsChromaMode => chromaMode;
        public bool IsHudVisible => hudVisible;
        public bool IsFeedVisible => feedVisible;
        public bool AreControlsVisible => controlsVisible;

        private const int DefaultWindowWidth = 540;
        private const int DefaultWindowHeight = 960;
        private int windowedWidth;
        private int windowedHeight;

        // Ghi nhớ kích thước cửa sổ ngay khi game mở, để tắt fullscreen là trở về
        // đúng khung dọc ban đầu chứ không giữ lại độ phân giải của fullscreen.
        ...
        // Bật: phủ kín màn hình theo đúng tỉ lệ màn hình, nhờ vậy OBS capture đầy canvas.
        // Tắt: trả lại nguyên kích thước cửa sổ trước đó, không đổi tỉ lệ hay độ phân giải.
        public void ApplyFullscreen(bool enabled)
        {
            if (enabled)
            {
                ...
                int width = Mathf.Max(native.width, Display.main.systemWidth);
                int height = Mathf.Max(native.height, Display.main.systemHeight);
                Screen.SetResolution(width, height, FullScreenMode.FullScreenWindow);
            }
            else
            {
                int width = windowedWidth > 0 ? windowedWidth : DefaultWindowWidth;
                int height = windowedHeight > 0 ? windowedHeight : DefaultWindowHeight;
        ...
        private void Update()
        {
            if (!Screen.fullScreen) RememberWindowSize();
            ...
            if (Input.GetKeyDown(KeyCode.F4)) feedVisible = !feedVisible;
            if (Input.GetKeyDown(KeyCode.F11)) ApplyFullscreen(!Screen.fullScreen);
        ...
        private void HandleEvent(TikTokEvent liveEvent)
        {
            ...
            if (liveEvent.type == "game_control")
            {
                ApplyRemoteGameControl(liveEvent);
                return;
            }
            ...
            if (requestsFocus && (liveEvent.type is "gift" or "chat" or "follow" or "share"))
            {
                ...
                // Rule đặt "Chỉ khi đang Top 1-3" thì không kéo camera cho người ngoài Top.
                if (!PlayerManager.ActionAllowed(liveEvent, actor)) return;
        ...
        private void OnGUI()
        {
            ...
            if (feedVisible) DrawFeed(height);
            if (controlsVisible) DrawControls();

            if (feedVisible && giftEffects != null && Time.unscaledTime < giftEffects.BannerUntil)
        ...
        private void DrawHeader(float width)
        {
            ...
            GUI.Label(new Rect(34, 23, 245, 28), "TISO", titleStyle);
        ...
        private void ToggleChroma() => SetChroma(!chromaMode);

        private void SetChroma(bool enabled)
        {
            chromaMode = enabled;
            ...
            // The club scene intentionally starts with fog disabled. Re-enabling fog
            // when leaving chroma mode washes the entire venue into a grey veil.
            RenderSettings.fog = false;
            if (Camera.main != null) Camera.main.backgroundColor = chromaMode ? new Color(0f, 1f, 0f) : new Color(0.01f, 0.005f, 0.025f);
        }

        private void ApplyRemoteGameControl(TikTokEvent liveEvent)
        {
            switch (liveEvent.command)
            {
                case "chroma":
                    SetChroma(liveEvent.boolValue);
                    break;
                case "hud":
                    hudVisible = liveEvent.boolValue;
                    break;
                case "feed":
                    feedVisible = liveEvent.boolValue;
                    break;
                case "controls":
                    controlsVisible = liveEvent.boolValue;
                    restoreControlsAfterCinematic = false;
                    break;
                case "fullscreen":
                    ApplyFullscreen(liveEvent.boolValue);
                    break;
                case "background_reload":
                    ClubSceneBuilder.ReloadBackground();
                    break;
                case "music_reload":
                {
                    MusicPlaylistPlayer music = FindFirstObjectByType<MusicPlaylistPlayer>();
                    music?.ReloadFromDisk();
                    break;
                }
                case "music_volume":
                {
        ...
        private void OnDestroy()
        {
            ...
            if (playerManager != null) playerManager.SnapshotApplied -= UpdateTopPlayers;
        }
    }
}
