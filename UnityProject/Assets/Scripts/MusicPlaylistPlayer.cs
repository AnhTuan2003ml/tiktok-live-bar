...
namespace TikTokLiveGame
{
    public sealed class MusicPlaylistPlayer : MonoBehaviour
    {
        ...
        private void Awake()
        {
            // Control Panel chạy trên trình duyệt nên game thường xuyên mất focus:
            // không bật cờ này thì Unity dừng cả update lẫn tiếng.
        ...
        public void ReloadFromDisk()
        {
            StopAllCoroutines();
            StopCurrentClip();
            tracks = FindTracks(out selectedFolder);
            trackIndex = 0;
        ...
        private void Update()
        {
            ...
            {
                wasPlaying = true;
                // Chỉ fade khi mới bắt đầu. Không fade cuối bài: bài phải phát trọn vẹn
                // rồi mới lặp lại hoặc sang bài kế.
                float fadeIn = Mathf.Clamp01((Time.unscaledTime - trackStartedAt) / FadeSeconds);
                output.volume = configuredVolume * fadeIn;
                watchdogAt = Time.unscaledTime + 3f;
            }
            ...
            else if (!loading && !advancePending && !output.loop && Time.unscaledTime >= watchdogAt)
            {
        ...
        private IEnumerator LoadAndPlay(string path)
        {
            ...
            // Đúng một bài: AudioSource lặp thẳng, hết bài là quay lại đầu ngay,
            // không phải tải lại file nên không có khoảng lặng giữa hai vòng.
            output.loop = tracks.Length == 1;
            output.volume = 0f;
        ...
        private static string[] FindTracks(out string selectedFolder)
        {
            ...
            string[] folders =
            {
                ...
                Path.Combine(projectRoot, "assets", "music"),
            ...
            foreach (string folder in folders)
            {
                ...
                string selectedPath = Path.Combine(folder, "SELECTED.txt");
                if (File.Exists(selectedPath))
                {
                    string selectedName = File.ReadAllText(selectedPath).Trim();
                    string candidate = Path.Combine(folder, selectedName);
                    if (File.Exists(candidate) && extensions.Contains(Path.GetExtension(candidate), StringComparer.OrdinalIgnoreCase))
                    {
