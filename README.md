# 🎵 TISO — TikTok Live 3D Dance Floor
...
[![Xem demo TISO trên TikTok LIVE](docs/media/live-demo.png)](https://www.facebook.com/100004611824062/videos/pcb.3562459197251106/37620662724214707)
...
1. Tải [`TISO-Live-Windows-v1.0.5.zip`](https://github.com/cherry9001/tiktok-live-bar/releases/latest/download/TISO-Live-Windows-v1.0.5.zip) trong mục **Releases**.
...
- Cửa sổ **TikTok Bridge** hiển thị địa chỉ theo `PORT` trong `TikTokBridge/.env` (mặc định `http://127.0.0.1:3100`).
- Trình duyệt mở Control Panel và logo TISO hiển thị bình thường.
...
chạy `npm ci`, Bridge/Control Panel/WebSocket hoạt động và game tự kết nối đúng cổng Bridge
thông qua tham số `-bridgePort` khi khởi động.

> `run.bat` không tự tắt chương trình khác đang dùng cổng Bridge (mặc định 3100). Nếu launcher báo
...
Truy cập [http://127.0.0.1:3100/control.html](http://127.0.0.1:3100/control.html) (theo `PORT` trong `TikTokBridge/.env`) trên trình duyệt.
...
1. Chọn nguồn kết nối trong Control Panel (tab Live): **Tự động**, **TikTok trực tiếp** hoặc **TikFinity Desktop**
2. Nhập username TikTok đang live → **Kết nối**
3. Với nguồn **TikFinity**: mở TikFinity Desktop → đăng nhập → bật LIVE (WebSocket cổng `21213`)

Chế độ **Tự động** thử kết nối thẳng tới TikTok trước; nếu không tìm thấy phiên LIVE
thì tự chuyển sang TikFinity.
...
`LIVE_PROVIDER` chọn nguồn kết nối TikTok LIVE:

- `auto` (mặc định) — tự thử kết nối **trực tiếp** tới TikTok bằng username trước;
  nếu tài khoản không tìm thấy/Live đã kết thúc thì tự động chuyển sang TikFinity Desktop.
- `tiktok` — kết nối thẳng tới TikTok qua username, **không cần mở TikFinity**.
- `tikfinity` — kết nối qua TikFinity Desktop (cần mở TikFinity, bật WebSocket cổng `21213`).

Trên Control Panel (tab Live) bạn cũng có thể chọn nguồn bằng ô **"Nguồn kết nối"**
khi bấm **Kết nối**, không cần sửa `.env`.

Bridge thực sự nạp file `.env` khi khởi động. Biến môi trường của Windows được ưu
tiên nếu cùng tên. `run.bat` đọc `PORT` từ `.env` và truyền `-bridgePort <PORT>` cho
game khi khởi động, nên bạn chỉ cần đổi `PORT` trong `.env` là toàn bộ Bridge,
Control Panel và game tự chuyển sang cổng mới mà **không cần build lại Unity**.
Khi mở game trực tiếp (không qua `run.bat`), game dùng cổng mặc định `3100`.
...
- **Cổng Bridge đang bị chiếm:** đóng đúng ứng dụng/PID được launcher báo; launcher không tự tắt ứng dụng khác.
...
1. Thêm source **Game Capture** → chọn cửa sổ TISO
...
<p align="center">
  Made with ❤️ by <strong>TISO</strong>
</p>
