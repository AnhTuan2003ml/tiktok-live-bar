...
const ACTIONS = new Set(['join', 'dance', 'jump', 'camera', 'change', 'walk', 'grow', 'medal', 'vip', 'topdj', 'fireworks']);

function sanitizeAction(value) {
    const action = String(value || '').trim().slice(0, 64);
    if (!action) return 'dance';
    if (ACTIONS.has(action)) return action;
    return /^[a-zA-Z0-9_.:-]+$/.test(action) ? action : 'dance';
}
...
function sanitizeRule(input = {}) {
    ...
    return {
        ...
        match: input.match === 'contains' ? 'contains' : input.match === 'any' ? 'any' : 'exact',
...
function matchesRule(rule, event) {
    ...
    const triggers = splitTriggers(rule.trigger);
    if (triggers.length === 0) return false;
    return triggers.some(trigger => rule.match === 'contains' ? value.includes(trigger) : value === trigger);
}
...
function applyRule(event, rule) {
    ...
    return {
        ...
        label: rule.label,
        decorationCondition: rule.decorationCondition,
